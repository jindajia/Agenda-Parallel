...
public:
    ...
    static void show(bool debug = false) {
		...
		if((int) timeUsed.size()>11){
			auto total = timeUsed[11] / TIMES_PER_SEC + timeUsed[3] / TIMES_PER_SEC;
		}
		...
		double total;
		for (int i = 1; i < (int) timeUsed.size(); i++) {
            if (timeUsed[i] > 0) {
				total+= timeUsed[i] / TIMES_PER_SEC;
            }
        }
		//INFO(total);
    }
